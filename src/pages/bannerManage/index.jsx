...
class Banner extends Component {
    ...
    handleOk = () => {
        ...
        form.validateFields((err, fieldsValue) => {
            ...
            // 直接修改fieldsValue，ESLint报错，所以深拷贝
            const fieldsValueClone = JSON.parse(JSON.stringify(fieldsValue))
            // 编辑 新增
            if (currentItem && currentItem.id) {
                fieldsValueClone.id = currentItem.id;
            }

            // 图片
            fieldsValueClone.url = fileList && fileList[0] ? fileList[0].url : null;
            fieldsValueClone.status = fieldsValueClone.status ? '1' : '0';

            // 新增编辑banner
            const uploadBanner = (url) => {

                if (url) {
                    fieldsValueClone.url = url;
                }
            ...
            <PageHeaderWrapper>
                <div className={styles.bannerManage}>
                                            ...
                                            <Carousel autoplay>
                                                {
                                                    ...
                                                    previewBanner.lb.map((item, index) => (
                                                        <a href={item.link} rel="noreferrer" target="_blank" key={`${index + item.link}`}>
                                                            <div className={styles.bannerItem} style={{ backgroundImage: `url(${item.url})` }} />
                                                        </a>
                                                    ))
                                                }
                                            </Carousel>
                                        ...
                                        <div className={styles.rightBanner}>
                                            {
                                                ...
                                                previewBanner.tj.map((item, index) => (
                                                    <a href={item.link} rel="noreferrer" target="_blank" key={`${index + item.link}`} className={styles.bannerItem} style={{ backgroundImage: `url(${item.url})` }}>
                    ...
                    <Card
                        ...
                        extra={extraContent}
                    >
                        {/* ~~对于正数向下取整，相当于Math.floor()；对于负数向上取整；对于非数字取值为0 */}
                        <Button
                            type="dashed"
